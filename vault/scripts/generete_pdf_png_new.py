...
def convert_pdf_to_png_optimized(pdf_file, png_output_path, page_w_mm, page_h_mm):
    """Convert PDF to PNG with optimized quality settings to reduce file size"""
    ...
    # Use optimized pixmap settings for smaller file size
    pix = page.get_pixmap(matrix=mat, alpha=False)  # No alpha channel if not needed
    
    # Save with PNG compression
    pix.save(str(png_output_path), output="png")
    doc.close()

def compress_png_in_memory(png_path):
    """Compress PNG using pngquant, processing in a separate thread"""
    try:
        # Use pngquant with optimized settings
        ...
        return f"✓ Compressed: {png_path.name}"
    except subprocess.CalledProcessError as e:
        return f"⚠ pngquant failed for {png_path.name}: {e.stderr.strip()}"
    except FileNotFoundError:
        return f"⚠ pngquant not found. Skipping compression for {png_path.name}"

def process_single_svg(svg_path):
    """Process a single SVG file - can be run in parallel"""
    ...
    if pdf_path.exists():
        return f"⏭ Skipping existing: {pdf_path.name}"

    try:
        thread_safe_print(f"🔄 Processing: {svg_path.name}")
        ...
        # Use optimized PNG conversion
        convert_pdf_to_png_optimized(pdf_path, png_path, page_w_mm, page_h_mm)
        
        # Compress PNG
        compression_result = compress_png_in_memory(png_path)
        
        return f"✅ Completed: {base_name} | {compression_result}"

    except Exception as e:
        return f"❌ Error processing {svg_path.name}: {e}"
...
def main():
    ...
    # Option 1: Sequential processing (original approach, but optimized)
    print("\n=== Sequential Processing ===")
    for svg_path in svg_files:
        result = process_single_svg(svg_path)
        print(result)
    
    # Option 2: Parallel processing (uncomment to use instead)
    # print("\n=== Parallel Processing ===")
    # with ThreadPoolExecutor(max_workers=4) as executor:
    #     # Submit all jobs
    #     future_to_svg = {executor.submit(process_single_svg, svg_path): svg_path 
    #                      for svg_path in svg_files}
    #     
